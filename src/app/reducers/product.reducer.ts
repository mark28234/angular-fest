    ...
    switch(action.type) {
        case productActions.LOAD_PRODUCT_SUCCESS:
           if(action.searchTerm){
               action.payload = action.payload.filter( obj => obj.name.indexOf(action.searchTerm)!= -1)
           }
           if (action.categories) {
            action.payload = action.payload.filter( obj => action.categories.includes(obj.categoryId.toString()))
           }
